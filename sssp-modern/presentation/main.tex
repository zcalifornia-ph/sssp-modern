...
\begin{frame}{DMMSY Centerpiece -- To Be Filled}
  \begin{block}{Planned content}
    The DMMSY centerpiece will add:
    \begin{itemize}
      \item BMSSP recursion (Algorithm 3) and FindPivots (Algorithm 1).
      \item Block-list (Lemma 3.3) amortization argument.
      \item Parameter derivation: $k = \lfloor \log^{1/3} n \rfloor$, $t = \lfloor \log^{2/3} n \rfloor$.
      \item Complexity proof sketch landing at $O(m \log^{2/3} n)$.
      \item Comparison-addition fidelity note (NFR-E).
    \end{itemize}
  \end{block}

  \vskip 6pt
  Implementation already lives at \texttt{src/sssp/dmmsy/}; source paper~\cite{duan2025breaking}.
\end{frame}
% Speaker note (California): centerpiece owner; a later content pass will populate.
% Until then this placeholder keeps the outline complete.
