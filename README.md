...
<h3 align="center">sssp-modern</h3>
  ...
  <p align="center">
    ...
    Version: v0.1.1
    <br />
    Status: Foundation implementation (graph primitives, weight checks, IO fixtures, and tests)
    <br />
...
Status: foundation implementation (`v0.1.1`). The repository now includes the first pure-stdlib Python primitives used by the planned shortest-path implementations: weighted graph structures, a comparison-addition `Weight` wrapper, and deterministic JSON graph fixtures.
...
3. Run the current foundation tests.
   ...
   ```sh
   cd sssp-modern
   PYTHONPATH=src python -m pytest tests/test_graph.py tests/test_weights.py tests/test_io.py
   ```
...
- `sssp-modern/src/sssp/`: pure-stdlib Python graph and weight primitives for reference implementations.
...
- [ ] v0.1.2 - Deterministic random graph generators for benchmark inputs.
