...
16. Rebuild the presentation deck PDF when needed.
    ...
    The current deck build produces `presentation/main.pdf` (27 pages) from the polished, public-facing Beamer source and `presentation/references.bib`.
...
- `sssp-modern/presentation/`: UP Beamer presentation source, bibliography, theme assets, logos, and compiled deck PDF; the current deck includes the title/outline spine, problem framing, baseline-algorithm slides, worked Dijkstra heap-relaxation and DMMSY bounded-batch visual walkthroughs, modern directed-sparse SSSP centerpiece slides, benchmark-results slides, a result-evidence slide in the main talk flow with a repository citation, reflection and integrity framing, author contributions, references, closing frame, final copy-fit/layout polish, and public-facing terminology cleanup.
